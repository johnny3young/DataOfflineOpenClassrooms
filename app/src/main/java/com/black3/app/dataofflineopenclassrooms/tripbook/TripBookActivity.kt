...
class TripBookActivity : AppCompatActivity() {
    ...
    if (radioButtonExternal.isChecked) {
       ...
       StorageUtils.setTextInStorage(
            Environment.getExternalStoragePublicDirectory(
            Environment.DIRECTORY_DOCUMENTS),this, FILENAME,FOLDERNAME,this.editText.text.toString())

        } else {
            StorageUtils.setTextInStorage(getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS),this,FILENAME,FOLDERNAME,this.editText.text.toString())
        }
